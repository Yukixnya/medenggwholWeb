...
//
// const ICON_MAP = {
//   favorite:'favorite', psychology:'psychology', self_improvement:'self_improvement',
//   monitor_weight:'monitor_weight', medical_services:'medical_services',
//   emergency:'emergency', sentiment_very_dissatisfied:'sentiment_very_dissatisfied',
// }
//
// const PALETTES = [
//   { from:'#E8F5E9', to:'#C8E6C9', icon:'#2E7D32', accent:'#43A047', text:'#1B5E20' },
//   { from:'#E3F2FD', to:'#BBDEFB', icon:'#1565C0', accent:'#1976D2', text:'#0D47A1' },
//   { from:'#FCE4EC', to:'#F8BBD0', icon:'#AD1457', accent:'#C2185B', text:'#880E4F' },
//   { from:'#FFF3E0', to:'#FFE0B2', icon:'#E65100', accent:'#F57C00', text:'#BF360C' },
//   { from:'#F3E5F5', to:'#E1BEE7', icon:'#6A1B9A', accent:'#7B1FA2', text:'#4A148C' },
//   { from:'#E0F7FA', to:'#B2EBF2', icon:'#00695C', accent:'#00796B', text:'#004D40' },
//   { from:'#FFF8E1', to:'#FFECB3', icon:'#F57F17', accent:'#F9A825', text:'#E65100' },
//   { from:'#EDE7F6', to:'#D1C4E9', icon:'#4527A0', accent:'#512DA8', text:'#311B92' },
// ]
//
...
//     setUserData({ selectedCategoryId:cat.id, selectedCategoryName:cat.name,
...
//         .hdr  { animation: fadeDown 0.5s ease both; }
//         .grid { animation: fadeUp 0.5s 0.15s ease both; }
//         .cat-card:hover { transform: translateY(-3px) !important; }
//       `}</style>
//
//         {/* ── Header ── */}
...
//           <div style={{ position:'absolute',bottom:-30,left:'35%',width:120,height:120,borderRadius:'50%',
//             background:'rgba(255,255,255,0.04)',pointerEvents:'none' }} />
//
//           <div style={{ maxWidth:640, margin:'0 auto', position:'relative', zIndex:1 }}>
//             <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
//               <div>
//                 <p style={{ color:'rgba(255,255,255,0.6)', fontSize:13, margin:'0 0 3px', letterSpacing:0.3 }}>
//                   Hello, {userData?.name || 'there'} 👋
//                 </p>
//                 <h1 style={{ color:'#fff', fontSize:21, fontWeight:800, margin:0, letterSpacing:-0.3 }}>
//                   How can we help?
//                 </h1>
//               </div>
...
//               }}>
//                 <span className="material-icons" style={{ color:'#fff', fontSize:20 }}>person</span>
//               </button>
//             </div>
//
//             {/* Search bar */}
...
//               <span className="material-icons" style={{ color:'rgba(255,255,255,0.6)', fontSize:18 }}>search</span>
//               <span style={{ color:'rgba(255,255,255,0.45)', fontSize:14 }}>Search health concerns...</span>
//             </div>
//
//             {/* Stats row */}
//             <div style={{ display:'flex', gap:12, marginTop:16 }}>
//               {[
//                 { icon:'grid_view', label:`${healthCategories?.length||0} Categories` },
//                 { icon:'verified_user', label:'Verified Doctors' },
//                 { icon:'flash_on', label:'Instant Help' },
//               ].map(item => (
//                   <div key={item.label} style={{ display:'flex', alignItems:'center', gap:5,
//                     background:'rgba(255,255,255,0.1)', borderRadius:20, padding:'5px 10px' }}>
//                     <span className="material-icons" style={{ fontSize:13, color:'rgba(255,255,255,0.8)' }}>{item.icon}</span>
//                     <span style={{ fontSize:11, color:'rgba(255,255,255,0.7)', fontWeight:600 }}>{item.label}</span>
//                   </div>
//               ))}
//             </div>
//           </div>
...
//
//         {/* ── Section label ── */}
//         <div style={{ maxWidth:640, margin:'0 auto', width:'100%', padding:'18px 20px 10px', boxSizing:'border-box' }}>
//           <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
//             <div style={{ display:'flex', alignItems:'center', gap:8 }}>
//               <div style={{ width:3, height:18, borderRadius:2,
//                 background:'linear-gradient(180deg,#2979FF,#0097A7)' }} />
//               <span style={{ fontSize:15, fontWeight:800, color:'#0A1628' }}>All Categories</span>
//             </div>
//             <span style={{ fontSize:12, color:'#6B7280', background:'#E8EAF6',
...
//
//         {/* ── Grid ── */}
//         <div className="grid" style={{ flex:1, overflowY:'auto', padding:'4px 20px 24px',
//           maxWidth:640, margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
//           <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:14 }}>
//             {(healthCategories||[]).map((cat,idx) => (
//                 <CategoryCard key={cat.id} cat={cat} palette={PALETTES[idx%PALETTES.length]}
...
//
//         {/* ── Bottom Nav ── */}
//         <div style={{ background:'#fff', borderTop:'1px solid #E8EAF6',
//           padding:'8px 0 16px', display:'flex', justifyContent:'space-around',
//           boxShadow:'0 -4px 24px rgba(10,22,40,0.08)', flexShrink:0 }}>
//           {[
//             { icon:'home_filled', label:'Home', id:'home' },
//             { icon:'history',     label:'History', id:'history' },
//             { icon:'notifications_none', label:'Alerts', id:'alerts' },
//             { icon:'person_outline', label:'Profile', id:'profile' },
//           ].map(item => {
//             const active = activeTab === item.id
//             return (
//                 <button key={item.id}
//                         onClick={() => { setActiveTab(item.id); item.id==='profile' && nav('/profile') }}
//                         style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2,
//                           background:'none', border:'none', cursor:'pointer', padding:'6px 20px', position:'relative' }}>
...
//                     color: active?'#2979FF':'#9CA3AF' }}>{item.icon}</span>
//                   <span style={{ fontSize:10, fontWeight: active?700:400,
//                     color: active?'#2979FF':'#9CA3AF', letterSpacing:0.2 }}>{item.label}</span>
//                 </button>
//             )
//           })}
//         </div>
...
// }
//
// function CategoryCard({ cat, palette, pressed, onPress, onRelease }) {
...
//                 borderRadius:20, border:`1px solid ${palette.icon}22`,
//                 padding:0, overflow:'hidden', cursor:'pointer', textAlign:'left',
//                 transform: pressed?'scale(0.95)':'scale(1)',
//                 transition:'transform 0.12s ease, box-shadow 0.2s ease',
//                 boxShadow: pressed
//                     ? `0 2px 8px ${palette.accent}44`
//                     : `0 6px 20px ${palette.accent}33`,
//                 display:'flex', flexDirection:'column',
//               }}>
//
//         {/* Image area - FULL image shown */}
//         <div style={{ width:'100%', height:120, position:'relative',
//           overflow:'hidden', flexShrink:0 }}>
//           {/* gradient overlay so image blends nicely */}
//           <div style={{ position:'absolute', inset:0, zIndex:1,
//             background:`linear-gradient(180deg,transparent 40%,${palette.from}CC 100%)` }} />
//
//           {imgSrc
...
//                 background:`linear-gradient(135deg,${palette.accent}22,${palette.from})`,
//                 display:'flex', alignItems:'center', justifyContent:'center' }}>
//                 <span className="material-icons" style={{ fontSize:52, color:palette.icon, opacity:0.4 }}>{icon}</span>
//               </div>
//           }
//
...
//             <span className="material-icons" style={{ fontSize:20, color:palette.icon }}>{icon}</span>
//           </div>
//
//           {/* subcategory badge */}
//           {cat.subcategories?.length > 0 && (
//               <div style={{ position:'absolute', top:8, right:8, zIndex:2,
//                 background:palette.icon, color:'#fff',
...
//           <p style={{ fontSize:13, fontWeight:800, color:palette.text,
//             margin:'0 0 3px', lineHeight:1.3 }}>
//             {cat.name}
//           </p>
//           {cat.description && (
//               <p style={{ fontSize:11, color:palette.icon, margin:'0 0 6px', opacity:0.7,
...
//           <span style={{ fontSize:9, fontWeight:800, color:palette.icon,
//             letterSpacing:0.8, opacity:0.9 }}>EXPLORE</span>
//             <span className="material-icons" style={{ fontSize:11, color:palette.icon }}>arrow_forward</span>
//           </div>
//         </div>
//       </button>
//   )
